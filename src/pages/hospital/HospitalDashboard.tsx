...
const HospitalDashboard: React.FC = () => {
    ...
    if (isLoading) {
        return (
            <div className={`min-h-screen p-6 ${isDarkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-blue-50 via-white to-indigo-50'}`}>
                <div className="flex items-center justify-center min-h-[60vh]">
                    <div className="text-center">
                        <LoadingSpinner size="lg" />
                        <p className={`mt-4 ${isDarkMode ? 'text-white' : 'text-slate-600'}`}>Loading your dashboard...</p>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-blue-50 via-white to-indigo-50'}`}>
            {/* Header Section */}
            <div className={`${isDarkMode ? 'bg-gray-800/90' : 'bg-white/80'} backdrop-blur-sm border-b ${isDarkMode ? 'border-gray-700' : 'border-slate-200'} sticky top-0 z-40`}>
                <div className="max-w-7xl mx-auto px-6 py-6">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            ...
                            <p className={`mt-1 font-medium ${isDarkMode ? 'text-gray-300' : 'text-slate-700'}`}>Manage patient access and view medical records</p>
                        </div>
                        <Button 
                            onClick={() => setShowRequestModal(true)}
                            className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 shadow-lg"
                        >
                            <svg className={`w-5 h-5 mr-2 ${isDarkMode ? 'text-white stroke-white' : 'text-white'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" style={isDarkMode ? {color: '#ffffff', stroke: '#ffffff'} : {}}>
            ...
            <div className="max-w-7xl mx-auto p-6">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                    <Card className={`border-0 shadow-xl ${isDarkMode ? 'bg-gray-800/70' : 'bg-white/70'} backdrop-blur-sm`}>
                        <CardHeader className="pb-6">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                <div>
                                    <CardTitle className={`text-2xl flex items-center gap-3 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                                        <div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg flex items-center justify-center">
                                            <svg className={`w-5 h-5 ${isDarkMode ? 'text-white stroke-white' : 'text-white'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" style={isDarkMode ? {color: '#ffffff', stroke: '#ffffff'} : {}}>
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                                            </svg>
                                        </div>
                                        <span className={`${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Patients with Active Access</span>
                                    </CardTitle>
                                    <CardDescription className={`mt-2 font-medium ${isDarkMode ? 'text-gray-300' : 'text-slate-700'}`}>
                        ...
                        <CardContent>
                            {filteredPatients.length === 0 ? (
                                ...
                                >
                                    <div className={`w-24 h-24 ${isDarkMode ? 'bg-gray-700' : 'bg-blue-100'} rounded-full flex items-center justify-center mx-auto mb-6`}>
                                        <svg className={`w-12 h-12 ${isDarkMode ? 'text-blue-400 stroke-blue-400' : 'text-blue-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" style={isDarkMode ? {color: '#60a5fa', stroke: '#60a5fa'} : {}}>
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                                        </svg>
                                    </div>
                                    <h3 className={`text-xl font-bold mb-3 ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>No Patients Found</h3>
                                    <p className={`mb-6 leading-relaxed max-w-md mx-auto font-medium ${isDarkMode ? 'text-gray-300' : 'text-slate-700'}`}>
                                                ...
                                                <Card className={`group hover:shadow-2xl transition-all duration-300 border-0 ${isDarkMode ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gradient-to-br from-white to-slate-50 hover:from-white hover:to-white'}`}>
                                                    ...
                                                    <CardContent className="pt-0">
                                                        <div className="space-y-3">
                                                            ...
                                                            <div className="flex justify-between text-sm">
                                                                <span className={`font-medium ${isDarkMode ? 'text-gray-300' : 'text-slate-700'}`}>Expires:</span>
                                                            ...
                                                            <div className={`pt-4 border-t ${isDarkMode ? 'border-gray-600' : 'border-slate-100'}`}>
                                                                <Button 
                                                                    ...
                                                                    size="sm"
                                                                >
                                                                    <svg className={`w-4 h-4 mr-2 ${isDarkMode ? 'text-white stroke-white' : 'text-white'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" style={isDarkMode ? {color: '#ffffff', stroke: '#ffffff'} : {}}>
